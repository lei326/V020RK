			...
			if (m_updatetime[camChn])
			{
				...
				if ((curtime > m_updatetime[camChn] ? curtime - m_updatetime[camChn] : m_updatetime[camChn] - curtime) > 500 * 1000)
				{
					...
					case L_CH16_FULL_SCREEN:
						// MSLOG_DEBUG("Clear!!!!");
						if (camChn == GetCurrentChannel() - L_CH10_FULL_SCREEN + 1)
							Hqt_Vivo_ClearChnVmix(0);
						break;
					case L_NINE_PICTURE:
						// MSLOG_DEBUG("Clear!!!");
					...
					// 清除9分割
					if ((GetCurrentChannel() & 0x800000FF) == 0x800000FF)
						Hqt_Vivo_ClearChnVmix(8);
...
int CIpCamManage::BindVdec(int32_t chn_num, int32_t *chn)
{
	...
	{
		if (Hqt_GetVDecEnable(vDecChn))
			Hqt_Dec_StopH264Vdec(vDecChn);
		if (chn_num == 1)
		{
			// 使用主码流
...
int CIpCamManage::GetVDecChn(int chn)
{
	int decChn = -1;
	// 原来是 >
	if (chn >= IPCCHANNELBEGIN + MAXIPCNUM)
		chn -= MAXIPCNUM; // 归一化到主/子共用键
	mapIpCamVDecIter iter = m_mapIpcVdec.find(chn);
	if (iter != m_mapIpcVdec.end())
		decChn = iter->second;
	return decChn;
}
...
static std::set<int> g_inited_rtmp_channels;
int CIpCamManage::StartIpCamStream(CIpCamStream *pSession, uint32_t chn)
{
	...
	pSession->IpCamStart();
 if (pSession->GetStreamType() == IPCAM_MAIN_STREAM)
    {
        int logicalCh = m_IpCamConfig[chn].Channel - 1;

        if (logicalCh >= 0)
        {
            if (g_inited_rtmp_channels.find(logicalCh) == g_inited_rtmp_channels.end())
            {
                char nameBuf[16];
                memset(nameBuf, 0, sizeof(nameBuf));
                snprintf(nameBuf, sizeof(nameBuf), "ch%02d", logicalCh);
                MSLOG_INFO("Init RTMP channel for IPC: id=%d name=%s", logicalCh, nameBuf);
                ms_rtmp_channel_init(logicalCh, nameBuf);
                g_inited_rtmp_channels.insert(logicalCh);
            }
            else
            {
                MSLOG_DEBUG("RTMP channel ch%02d already initialized, skip.", logicalCh);
            }
        }
        else
        {
            MSLOG_WARN("RTMP init skipped: logicalCh=%d < 0 for chn=%u", logicalCh, chn);
        }
    }
	return 0;
}

void* CIpCamManage::IpcSnapMemThreadProc_(void *arg) {
    IpcSnapMemCtx *ctx  = reinterpret_cast<IpcSnapMemCtx*>(arg);
    CIpCamManage  *self = ctx->self;

    ctx->jpg = nullptr; ctx->jpg_len = 0; ctx->result = -1;

    int vdecChn = self->GetVDecChn(ctx->absCh);
    if (vdecChn < 0) {
        int32_t one = ctx->absCh;
        (void)self->BindVdec(1, &one);             
        vdecChn = self->GetVDecChn(ctx->absCh);
        if (vdecChn < 0) {
            MSLOG_ERROR("IpcSnapMemThread: no VDEC for absCh=%d", ctx->absCh);
            return ctx;
        }
    }

    uint32_t w = 0, h = 0;
    if (self->GetChnSize(ctx->absCh, /*isMain*/true, &w, &h) < 0 || w == 0 || h == 0) {
        MSLOG_ERROR("IpcSnapMemThread: GetChnSize failed absCh=%d", ctx->absCh);
        return ctx;
    }
    const uint32_t vw = (w + 15) & ~15U;
    const uint32_t vh = (h + 15) & ~15U;

    (void)self->WaitVdecReadyIdx_(ctx->idx, 1500);
    usleep(200 * 1000); 
    ...
    if (vencChn < 0) {
        MSLOG_ERROR("IpcSnapMemThread: create VENC failed absCh=%d", ctx->absCh);
        return ctx;
    }

    VENC_JPEG_PARAM_S jpgParam; memset(&jpgParam, 0, sizeof(jpgParam));
    int q = (ctx->quality < 1) ? 1 : (ctx->quality > 100 ? 100 : ctx->quality);
    ...
    RGA_CHN rgaChn = -1;
    RGA_ATTR_S ra; memset(&ra, 0, sizeof(ra));
    ra.bEnBufPool = RK_TRUE; ra.u16BufPoolCnt = 3; ra.u16Rotaion = 0;
    ra.stImgIn.imgType      = IMAGE_TYPE_NV12;
    ra.stImgIn.u32X         = 0;
    ra.stImgIn.u32Y         = 0;
    ra.stImgIn.u32Width     = w;
    ra.stImgIn.u32Height    = h;
    ra.stImgIn.u32HorStride = vw;
    ra.stImgIn.u32VirStride = vh;
    ra.stImgOut = ra.stImgIn;

    for (int ch = 0; ch < RGA_MAX_CHN_NUM; ++ch) {
        if (RK_MPI_RGA_CreateChn(ch, &ra) == 0) { rgaChn = ch; break; }
    }
    if (rgaChn < 0) {
        MSLOG_ERROR("IpcSnapMemThread: create RGA failed absCh=%d", ctx->absCh);
    ...
    if (RK_MPI_SYS_Bind(&src, &mid) != 0) {
        MSLOG_ERROR("IpcSnapMemThread: Bind VDEC(%d)->RGA(%d) failed", vdecChn, rgaChn);
        RK_MPI_RGA_DestroyChn(rgaChn);
        RK_MPI_VENC_DestroyChn(vencChn);
        return ctx;
    }
    if (RK_MPI_SYS_Bind(&mid, &dst) != 0) {
        MSLOG_ERROR("IpcSnapMemThread: Bind RGA(%d)->VENC(%d) failed", rgaChn, vencChn);
    ...
    RK_MPI_VENC_StartRecvFrame(vencChn, &rp);
    RK_MPI_SYS_StartGetMediaBuffer(RK_ID_VENC, vencChn);
    MEDIA_BUFFER out = RK_MPI_SYS_GetMediaBuffer(RK_ID_VENC, vencChn, ctx->timeout_ms);
    if (!out) {
        MSLOG_ERROR("IpcSnapMemThread: JPEG timeout absCh=%d after %d ms", ctx->absCh, ctx->timeout_ms);
    ...
    if (!buf) {
        MSLOG_ERROR("IpcSnapMemThread: malloc(%zu) failed", size);
    ...
    MSLOG_INFO("IPC SNAP(mem) OK: absCh=%d idx=%d vdec=%d len=%zu",
               ctx->absCh, ctx->idx, vdecChn, size);
    return ctx;
}


int CIpCamManage::SnapshotJpegMultiToMem(const int *absChs, int n, bool isMain, int timeout_ms, int quality, SnapMem **out_arr, int *out_n)
{
    if (!absChs || n <= 0 || !out_arr || !out_n) return -1;
    *out_arr = nullptr;
    *out_n   = 0;

    std::vector<SnapMem> outs;
    outs.reserve(n);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (absCh < IPCCHANNELBEGIN || absCh >= IPCCHANNELBEGIN + MAXIPCNUM) {
            MSLOG_WARN("SnapshotJpegMultiToMem(serial): drop invalid absCh=%d", absCh);
            continue;
        }

        (void)EnsureVdecForAbsCh_(absCh, /*isMain=*/true);
        const int idx = absCh - IPCCHANNELBEGIN;
        if (idx >= 0 && idx < MAXIPCNUM) m_u8NeedIFrame[idx] = 1; 
        (void)WaitVdecReadyIdx_(idx, 1200);
        usleep(150 * 1000);  
        ...
        (void)CIpCamManage::IpcSnapMemThreadProc_(&ctx);

        if (ctx.result == 0 && ctx.jpg && ctx.jpg_len > 0) {
            SnapMem sm;
            sm.channel = absCh;      
            sm.data    = ctx.jpg;   
            sm.len     = ctx.jpg_len;
            outs.push_back(sm);
            ++ok;
            MSLOG_INFO("SnapshotJpegMultiToMem(serial): absCh=%d len=%zu ok", absCh, sm.len);
        } else {
            if (ctx.jpg) std::free(ctx.jpg);
            MSLOG_ERROR("SnapshotJpegMultiToMem(serial): absCh=%d snap failed", absCh);
        }
    ...
    if (ok <= 0) {
        MSLOG_WARN("SnapshotJpegMultiToMem(serial): no success");
        return -2;
    }

    SnapMem *arr = (SnapMem*)std::calloc(outs.size(), sizeof(SnapMem));
    if (!arr) {
        for (auto &s : outs) if (s.data) std::free(s.data);
        MSLOG_ERROR("SnapshotJpegMultiToMem(serial): alloc arr failed");
        return -3;
    }
    for (size_t i = 0; i < outs.size(); ++i) arr[i] = outs[i];

    *out_arr = arr;
    *out_n   = (int)outs.size();
    MSLOG_INFO("SnapshotJpegMultiToMem(serial) done: total=%d, ok=%d", n, *out_n);
    return 0;
}


void CIpCamManage::FreeSnapMemArray(SnapMem *arr, int n) {
  if (!arr) return;
  for (int i = 0; i < n; ++i) {
    if (arr[i].data) std::free(arr[i].data);
  }
  std::free(arr);
}
...
int CIpCamManage::EnsureVdecForAbsCh_(int absCh, bool isMain)
{
	...
	const int idx = base;
	const int WAIT_MS_MAP = 1200;
	const int WAIT_MS_FRAME = 800;
