...
int absCh = 9;
mgr->BindVdec(1, &absCh);                  // 单通道 -> 绑定主码流；只需调用一次
usleep(200 * 1000);                        // 给解码出图一点时间（可选）

int ret = mgr->SnapshotJpeg(9 - IPCCHANNELBEGIN /*=0*/, true,
                            "/tmp/ipc9_main.jpg", 5000, 90);

return 0;


static int CreateJpegVencChnAuto(const VENC_CHN_ATTR_S& baseAttr, VENC_CHN& outChn) {
  for (int ch = VENC_MAX_CHN_NUM - 1; ch >= 0; --ch) {
    if (RK_MPI_VENC_CreateChn(ch, (VENC_CHN_ATTR_S*)&baseAttr) == 0) {
      outChn = ch;
      return 0;
    }
    if (RK_MPI_VENC_CreateJpegLightChn(ch, (VENC_CHN_ATTR_S*)&baseAttr) == 0) {
      outChn = ch;
      return 0;
    }
  }
...
int CIpCamManage::SnapshotJpeg(uint8_t chn, bool isMain, const char* jpg_path,int timeout_ms, int quality) {
  if (!jpg_path || !jpg_path[0]) {
    MSLOG_ERROR("SnapshotJpeg: invalid jpg_path");
    return -1;
  }
  int ipcamAbs = IPCCHANNELBEGIN + (isMain ? chn : (chn + MAXIPCNUM));

  int vdecChn = GetVDecChn(ipcamAbs);
  if (vdecChn < 0) {
  ...
  memset(&src, 0, sizeof(src)); memset(&mid, 0, sizeof(mid)); memset(&dst, 0, sizeof(dst));
  ...
  if (fd < 0) {
    MSLOG_ERROR("SnapshotJpeg: open('%s') failed", jpg_path);
  ...
  if (wr < 0 || (size_t)wr != size) {
    MSLOG_ERROR("SnapshotJpeg: write('%s') failed (%zd/%zu)", jpg_path, wr, size);
    return -1;
  }

  MSLOG_INFO("SnapshotJpeg OK: absChn=%d(idx=%d) isMain=%d vdec=%d => %s (%ux%u q=%d)",
             ipcamAbs, (int)chn, (int)isMain, vdecChn, jpg_path, w, h, q);
  return 0;
}
