...
public:
	int SnapshotJpeg(uint8_t chn, bool isMain,const char* jpg_path, int timeout_ms, int quality);
    ...
    int SnapshotJpegMultiToMem(const int *absChs, int n, bool isMain, int timeout_ms, int quality, SnapMem **out_arr, int *out_n);
    void FreeSnapMemArray(SnapMem *arr, int n);
protected:
...
private:
	...
	static void* IpcSnapMemThreadProc_(void *arg);
