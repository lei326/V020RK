...
int rk_get_incident_images(const IncidentImageRequest_t *request, IncidentImageResponse_t *response)
{
    ...
    int chs[] = {0};                
    const size_t n_ch = sizeof(chs)/sizeof(chs[0]);

    SnapResult *results = NULL;
    int rc = Doordvr_Snap_Multi_Base64(chs, (int)n_ch, PIC_D1, &results);
    if (rc != 0 || !results) {
        DVR_DEBUG("Doordvr_Snap_Multi_Base64 failed, rc=%d", rc);
        response->isValid = 0;
        response->error   = (char*)"snap to base64 failed";
        return 0;
    }

    for (size_t i = 0; i < n_ch; ++i) {
        bool ok = (results[i].b64 && results[i].b64_len > 0);
        if (!ok) {
            DVR_DEBUG("Snap ch%zu failed or empty", i);
            for (size_t j = 0; j < n_ch; ++j) {
                if (results[j].b64) free(results[j].b64);
            }
            free(results);
            response->isValid = 0;
            response->error   = (char*)"capture timeout or encode failed";
            return 0;
        }
    }

    for (size_t i = 0; i < n_ch; ++i) {
        if (results[i].b64 && results[i].b64_len > 0) {
            DVR_DEBUG("B64 HEAD ch%d: \"%.100s\" (total=%zu)",
                      results[i].channel, results[i].b64, results[i].b64_len);
        } else {
            DVR_DEBUG("B64 HEAD ch%d: <empty>", results[i].channel);
        }
    }

    response->imageCount = (int)n_ch;
    response->images = (IncidentImageInfo_t *)calloc(n_ch, sizeof(*response->images));
    if (!response->images) {
        for (size_t j = 0; j < n_ch; ++j) {
            if (results[j].b64) free(results[j].b64);
        }
    ...
    for (size_t i = 0; i < n_ch; ++i) {
        response->images[i].id       = results[i].channel;           
        response->images[i].file     = results[i].b64;
        response->images[i].fileSize = (int)results[i].b64_len;
        results[i].b64 = NULL;  
    }

    for (size_t i = 0; i < n_ch; ++i) {
        DVR_DEBUG("Image[%zu]: id=%d len=%d strlen=%zu",
                  i, response->images[i].id,
                  response->images[i].fileSize,
                  strlen(response->images[i].file));
    }
