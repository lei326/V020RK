...
static inline void safe_strcpy(char *dst, size_t cap, const char *src) {
    if (!dst || cap == 0) return;
...
#define SAFE_STRCPY_ARR(dst_array, src_cstr) \
    safe_strcpy((dst_array), sizeof(dst_array), (src_cstr))

#define SAFE_STRCPY_PTR(dst_ptr, cap, src_cstr) \
    safe_strcpy((dst_ptr), (cap), (src_cstr))

#define SAFE_STRCPY   SAFE_STRCPY_ARR
#define SAFE_STRCPY_N SAFE_STRCPY_PTR
...
static int kitid_validate_then_fill(const char *reqKidIn,
                                    const char *storedKidIn
                                    )
{
    char reqKid[64]    = {0};
    char storedKid[64] = {0};
...
int rk_get_incident_images(const IncidentImageRequest_t *request, IncidentImageResponse_t *response)
{
    ...
    int chs[] = {0 ,1};
    Doordvr_Snap_Multi(chs, 2, PIC_D1);
    ...
    SAFE_STRCPY(response->eventID, "111");
    SAFE_STRCPY(response->kitID, allConfig.AllConfigDef.abdparam.IdaCfgSet.createkitresponsedata.kitID);
    response->isValid = true;
    response->error   = NULL;

    const char *jpeg_path = "/opt/C807RK/123.jpg";
    unsigned char *bin = NULL;
    size_t bin_len = 0;
    int rf = read_file_all(jpeg_path, &bin, &bin_len);
    if (rf != 0) {
        DVR_DEBUG("read_file_all('%s') failed, err=%d", jpeg_path, rf);
        response->isValid = 0;
        response->error   = (char*)"read file failed";
        return 0;
    }
    DVR_DEBUG("Read JPEG: path=%s, size=%zu bytes", jpeg_path, bin_len);

    int jpeg_sig_ok = (bin_len >= 3 && bin[0] == 0xFF && bin[1] == 0xD8 && bin[2] == 0xFF);
    DVR_DEBUG("JPEG signature (FFD8FF) %s", jpeg_sig_ok ? "OK" : "MISSING");

    size_t b64_len = 0;
    char *b64 = base64_encode(bin, bin_len, &b64_len);
    free(bin);
    if (!b64) {
        DVR_DEBUG("base64_encode failed");
        response->isValid = 0;
        response->error   = (char*)"base64 encode failed";
        return 0;
    }
    DVR_DEBUG("Base64 length=%zu", b64_len);

    const char *hint = (b64_len >= 4 && b64[0] == '/' && b64[1] == '9' && b64[2] == 'j' && b64[3] == '/')
                       ? "LIKELY_JPEG" : "UNKNOWN_FMT";
    size_t show = (b64_len < 64) ? b64_len : 64;
    char head64[65];
    memcpy(head64, b64, show);
    head64[show] = '\0';
    DVR_DEBUG("Base64 len=%zu, head[%.0f]=\"%s\" (%s)", b64_len, (double)show, head64, hint);

    response->imageCount = 1;
    response->images = (IncidentImageInfo_t *)calloc(1, sizeof(*response->images));
    if (!response->images) {
        free(b64);
        response->imageCount = 0;
        response->isValid = 0;
        response->error   = (char*)"alloc images failed";
        return 0;
    }

    response->images[0].id       = 1;
    response->images[0].file     = b64;                  
    response->images[0].fileSize = (int)b64_len;  
    int size_ok = ((size_t)response->images[0].fileSize == strlen(response->images[0].file));
    DVR_DEBUG("IncidentImages OK: count=%d, id=%d, b64Len=%d, strlen(file)=%zu, size_match=%s",
              response->imageCount,
              response->images[0].id,
              response->images[0].fileSize,
              strlen(response->images[0].file),
              size_ok ? "YES" : "NO");
            const char *b64_str = b64;
    int rc = save_base64_to_file(b64_str, "/opt/C807RK/decoded_from_b64.jpg");
    DVR_DEBUG("Decode %s, rc=%d", (rc==0 ? "success" : "FAILED"), rc);          
    return 0;
}
...
int rk_get_device_info(const DeviceInfoRequest_t *request, DeviceInfoResponse_t *response) {
    ...
    SAFE_STRCPY(response->kitInfoDto.version, "default");
...
int rk_get_current_location(const CurrentLocationRequest_t *request, CurrentLocationResponse_t *response)
{
    ...
    int direction_deg = s_prev_dir;

    if (ok) {
        // 正确：把 double 按格式写入字符串
        snprintf(loc->latitude,  sizeof(loc->latitude),  "%.7f", lat_deg);
        snprintf(loc->longitude, sizeof(loc->longitude), "%.7f", lon_deg);
        loc->radius = r95_m;

        if (s_has_prev) {
            double dist_m = haversine_m(s_prev_lat, s_prev_lon, lat_deg, lon_deg);
            if (dist_m >= 0.1) {
                double brg = initial_bearing_deg(s_prev_lat, s_prev_lon, lat_deg, lon_deg);
                direction_deg = (int)lround(brg);
                if (direction_deg >= 360) direction_deg = 0;
                s_prev_dir = direction_deg;
                DVR_DEBUG("Computed heading: dist=%.2f m, brg=%.2f deg -> dir=%d",
                          dist_m, brg, direction_deg);
            } else {
                DVR_DEBUG("Movement too small (%.2f m), keep last dir=%d", dist_m, direction_deg);
            }
        } else {
            DVR_DEBUG("First valid fix, no previous point for heading; keep dir=%d", direction_deg);
            s_prev_dir = direction_deg;
            s_has_prev = 1;
        }
...
int rk_get_health_check(const HealthCheckRequest_t *request, HealthCheckResponse_t *response)
{
    ...
    for(int i = 0; i < response->healthCheckData.cameraCount; ++i){
        ...
        response->healthCheckData.cameraInfoDto[i].type = i;//default val
...
int rk_create_incident(const CreateIncidentRequest_t *request, CreateIncidentResponse_t *response)
{
    ...
    int cfg_cam_n = allConfig.AllConfigDef.abdparam.IdaCfgSet.createkitresponsedata.devices[0].cameraCount;
