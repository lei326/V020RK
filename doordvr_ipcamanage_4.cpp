...
int CIpCamManage::SnapshotJpegMulti(const int *absChs, int n, bool /*isMain*/, const char *save_dir, int timeout_ms, int quality)
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		long long first = m_updatetime[idx];
		bool ready = false;
		if (idx >= 0 && idx < MAXIPCNUM)
			m_u8NeedIFrame[idx] = 1;

		for (int t = 0; t < 20; ++t)
		{
			bool lost = GetVideoLost((uint8_t)absCh, true);
			if (!lost && m_updatetime[idx] != first)
			{
				ready = true;
				break;
			}
		...
		uint32_t w = 0, h = 0;
		if (GetChnSize(absCh, true, &w, &h) < 0 || w == 0 || h == 0)
		{
		...
		{
			VENC_CHN vencChn = -1;
			VENC_CHN_ATTR_S veAttr;
			memset(&veAttr, 0, sizeof(veAttr));
			...
			{
				if (RK_MPI_VENC_CreateChn(ch, &veAttr) == 0)
				{
					vencChn = ch;
					break;
				}
				if (RK_MPI_VENC_CreateJpegLightChn(ch, &veAttr) == 0)
				{
					vencChn = ch;
					break;
				}
			...
			VENC_JPEG_PARAM_S jpgParam;
			memset(&jpgParam, 0, sizeof(jpgParam));
			...
			src.enModId = RK_ID_VDEC;
			src.s32ChnId = vdecChn;
			dst.enModId = RK_ID_VENC;
			dst.s32ChnId = vencChn;

			if (use_rga)
			{
				RGA_ATTR_S rgaAttr;
				memset(&rgaAttr, 0, sizeof(rgaAttr));
				...
				{
					if (RK_MPI_RGA_CreateChn(ch, &rgaAttr) == 0)
					{
						rgaChn = ch;
						break;
					}
		...
		if (snap_once(false) || snap_once(true))
		{
			++ok;
		}
	}
...
void *CIpCamManage::IpcSnapThreadProc_(void *arg)
{
	...
	uint32_t w = 0, h = 0;
	if (self->GetChnSize(absCh, isMain, &w, &h) < 0 || !w || !h)
		return ctx;
	...
	VENC_CHN_ATTR_S ve{};
	ve.stVencAttr.enType = RK_CODEC_TYPE_JPEG;
	ve.stVencAttr.imageType = IMAGE_TYPE_NV12;
	ve.stVencAttr.u32PicWidth = w;
	ve.stVencAttr.u32PicHeight = h;
	ve.stVencAttr.u32VirWidth = vw;
	ve.stVencAttr.u32VirHeight = vh;
	VENC_CHN venc = -1;
	for (int ch = VENC_MAX_CHN_NUM - 1; ch >= 0; --ch)
	{
		if (RK_MPI_VENC_CreateChn(ch, &ve) == 0)
		{
			venc = ch;
			break;
		}
		if (RK_MPI_VENC_CreateJpegLightChn(ch, &ve) == 0)
		{
			venc = ch;
			break;
		}
	...
	ra.stImgOut.imgType = IMAGE_TYPE_NV12;
	ra.stImgOut.u32Width = w;
	ra.stImgOut.u32Height = h;
	ra.stImgOut.u32HorStride = vw;
	ra.stImgOut.u32VirStride = vh;
	RGA_CHN rga = -1;
	for (int ch = 0; ch < RGA_MAX_CHN_NUM; ++ch)
		if (RK_MPI_RGA_CreateChn(ch, &ra) == 0)
		{
			rga = ch;
			break;
		}
	...
	if (fd >= 0)
	{
		void *p = RK_MPI_MB_GetPtr(out);
		size_t sz = RK_MPI_MB_GetSize(out);
		ssize_t wr = ::write(fd, p, sz);
		(void)wr;
		::close(fd);
	}
	RK_MPI_MB_ReleaseBuffer(out);
