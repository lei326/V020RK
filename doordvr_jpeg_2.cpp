//增添功能：抓拍的时候，创建RGA通道，从RGA0开始创建，如果RGA0通道不可用，创建失败就会自动创建RGA1，直到创建的RGA通道能用为止
...
#ifndef RGA_MAX_CHN_NUM
#define RGA_MAX_CHN_NUM 4   // 如 SDK 有官方宏请改用官方宏
#endif
#define SNAP_RGA_BASE    0 
#define SNAP_RGA_MAX     RGA_MAX_CHN_NUM
// -----------------------------------
...
static int create_rga_round_robin(int preferred, RGA_ATTR_S *attr, int *outChn) {
    int last_ret = -1;
    for (int k = 0; k < RGA_MAX_CHN_NUM; ++k) {
        int idx = (preferred + k) % RGA_MAX_CHN_NUM;
        int ret = RK_MPI_RGA_CreateChn(idx, attr);
        if (ret == 0) {
            *outChn = idx;
            return 0;
        }
        last_ret = ret;
        DVR_DEBUG("[SNAP] try RGA[%d] failed, ret=0x%x", idx, ret);
    }
    return (last_ret == 0) ? -1 : last_ret;
}
...
static void video_packet_cb(MEDIA_BUFFER mb)
{
    ...
    if (ptr && sz > 0) {
        ...
        else if (b64) { free(b64); b64 = NULL; b64_len = 0; } 
    }
...
static void* single_snap_thread(void *arg)
{
    ...
    if (RK_MPI_VENC_CreateChn(ctx->vencChn, &venc_attr)) {
        server_printf("[SNAP][CH%d] Create VENC[%d] failed\n", ctx->srcChannel, ctx->vencChn);
        return NULL;
    }
    ...
    if (RK_MPI_SYS_RegisterOutCb(&encChn, video_packet_cb)) {
        server_printf("[SNAP][CH%d] RegisterOutCb failed\n", ctx->srcChannel);
    ...
    int preferred = (SNAP_RGA_BASE + ctx->srcChannel) % RGA_MAX_CHN_NUM; 
    int ret = create_rga_round_robin(preferred, &stRgaAttr, &ctx->rgaChn);
    if (ret != 0) {
        server_printf("[SNAP][CH%d] Create RGA failed (preferred=%d). last_ret=0x%x\n",
                      ctx->srcChannel, preferred, ret);
        RK_MPI_VENC_DestroyChn(ctx->vencChn);
        return NULL;
    }
    DVR_DEBUG("[SNAP][CH%d] use RGA[%d]", ctx->srcChannel, ctx->rgaChn);
    // ----------------------------------------------------------------------
    ...
    if (RK_MPI_SYS_Bind(&srcChn, &midChn)) {
        server_printf("[SNAP][CH%d] Bind VI->RGA failed\n", ctx->srcChannel);
        RK_MPI_RGA_DestroyChn(ctx->rgaChn);
        RK_MPI_VENC_DestroyChn(ctx->vencChn);
        return NULL;
    }
    if (RK_MPI_SYS_Bind(&midChn, &dstChn)) {
        server_printf("[SNAP][CH%d] Bind RGA->VENC failed\n", ctx->srcChannel);
    ...
    if (RK_MPI_VENC_StartRecvFrame(ctx->vencChn, &recvParam)) {
        server_printf("[SNAP][CH%d] StartRecvFrame failed\n", ctx->srcChannel);
    ...
    *out_results = NULL;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (vencChn < 0 || vencChn >= SNAP_VENC_MAX) {
            server_printf("[SNAP] invalid venc id %d for ch %d\n", vencChn, ch);
            continue;
        }
        ...
        if (ret != 0) {
            server_printf("[SNAP] pthread_create failed for ch %d: %d\n", ch, ret);
...
int Doordvr_Snap_StartJpeg(VI_CHN channel, PIC_SIZE_E picSize, void *param)
{
    ...
    if (ret) {
        server_printf("Create Venc failed! ret=%d\n", ret);
        return -1;
    }
    ...
    if (ret) {
        server_printf("Register Output callback failed! ret=%d\n", ret);
    ...
    int preferred = (SNAP_RGA_BASE + channel) % RGA_MAX_CHN_NUM;
    int rgaCh = -1;
    ret = create_rga_round_robin(preferred, &stRgaAttr, &rgaCh);
    if (ret) {
        server_printf("Create RGA failed! preferred=%d, last_ret=0x%x\n", preferred, ret);
        RK_MPI_VENC_DestroyChn(SNAP_ENCODER_CHANNEL);
        return -1;
    }
    DVR_DEBUG("[SNAP][CH%d] use RGA[%d]", channel, rgaCh);
    ...
    if (ret) {
        server_printf("Bind VI -> RGA failed! ret=%d\n", ret);
        RK_MPI_RGA_DestroyChn(rgaCh);
        RK_MPI_VENC_DestroyChn(SNAP_ENCODER_CHANNEL);
        return -1;
    }
    ret = RK_MPI_SYS_Bind(&stMidChn, &stDestChn);
    if (ret) {
        server_printf("Bind RGA -> VENC failed! ret=%d\n", ret);
    ...
    if (ret) {
        server_printf("RK_MPI_VENC_StartRecvFrame failed!\n");
